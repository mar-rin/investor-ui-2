...
const WebForm = () => {
  ...
  const [analysis, setAnalysis] = useState("Waiting for server response...");
  ...
  const investmentCriteria = Cookies.get('investmentCriteria'); // Retrieves the value of the cookie named 'myCookie'.
  useEffect(() => {
    setFormData({
      investment_criteria: Cookies.get("investmentCriteria") ? Cookies.get("investmentCriteria") : '',
      vertical_criteria: Cookies.get("investmentVerticals") ? Cookies.get("investmentVerticals") : '',
      ticket_size: Cookies.get("ticketSize") ? Cookies.get("ticketSize") : '',
      stage_focus: Cookies.get("stage") ? Cookies.get("stage") : '',
    });
  }, []);



  const handleSetCookie = (investmentCriteria, investmentVerticals, ticketSize, stage) => {
    Cookies.set('investmentCriteria', investmentCriteria, { expires: 7 }); // Sets a cookie named 'myCookie' with the value 'exampleValue' that expires in 7 days.
    Cookies.set('investmentVerticals', investmentVerticals, { expires: 7 }); // Sets a cookie named 'myCookie' with the value 'exampleValue' that expires in 7 days.
    Cookies.set('ticketSize', ticketSize, { expires: 7 }); // Sets a cookie named 'myCookie' with the value 'exampleValue' that expires in 7 days.
    Cookies.set('stage', stage, { expires: 7 }); // Sets a cookie named 'myCookie' with the value 'exampleValue' that expires in 7 days.
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    handleSetCookie(formData.investment_criteria, formData.vertical_criteria, formData.ticket_size, formData.stage_focus)
        ...
        <div>
          ...
          <div className="bg-off-white text-charcoal-gray text-center rounded-xl p-3 ">
            <form onSubmit={handleSubmit}>
              ...
              <div>
                <br></br>
                <p><b>Upload the Pitch Deck PDF to analyze: </b></p>
                <input
                    type="file"
